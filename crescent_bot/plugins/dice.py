...
class Dice:
    ...
    number = crescent.option(int, "The number of dice to roll.", min_value=1, max_value=25)

    # These two are the same type, but are optional. We can provide a
    # default value simply by using the `default` kwarg.
    sides = crescent.option(int, "The number of sides each die will have.", default=6, min_value=0, max_value=100)
    bonus = crescent.option(int, "A fixed number to add to the total roll.", default=0, min_value=0, max_value=100)
